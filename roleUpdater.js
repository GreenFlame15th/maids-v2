function login(user, fusionRoles, roles, log) {
    ...
    if(nsfw){
        fusionRoles.nsfwFusion.forEach(fu =>{
            if (userRoleIds.includes(fu[0])) {addById(fu[1])}
            else
                if (!userRoleIds.includes(fu[0])) {removeById(fu[1])}
            })
    }else
        fusionRoles.nsfwFusion.forEach(fu => {
            if (userRoleIds.includes(fu[1])) {removeById(fu[1])}
        })
    //vvv here goes date/fusion role handler vvv
    fusionRoles.fusion.forEach( f=>{
        if(f[1].every(r=>userRoleIds.includes(r))){addById(f[0])}
        else {removeById(f[0])}
    })
}
